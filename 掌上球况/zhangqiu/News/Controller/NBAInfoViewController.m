...
@interface NBAInfoViewController ()<UITableViewDataSource, UITableViewDelegate, UIWebViewDelegate>
...
@implementation NBAInfoViewController
- (void)dealloc
{
    [_news release];
    [_time release];
...
- (void)viewDidLoad {
    ...
    self.tableV = [[UITableView alloc] initWithFrame:CGRectMake(0, 64, myWidth, myHeight - 64) style:UITableViewStylePlain];
...

{
    ...
    self.webView.frame = CGRectMake(0,newCellHeight, myWidth, height + 100);
...
- (void)getDate
{
    ...
    NSString *url = @"";
//    if (nil == self.news) {
//        self.time = [self.NBAInfo.createtime substringToIndex:10];
//        url =[NSString stringWithFormat:@"http://m.zhibo8.cc/news/android/json/nba/%@/%@.json?aabbccddeeff=%ld", self.time, self.NBAInfo.filename, i];
//    } else {
        self.time = [self.news.createtime substringToIndex:10];
        url =[NSString stringWithFormat:@"http://m.zhibo8.cc/news/android/json/nba/%@/%@.json?aabbccddeeff=%ld", self.time, self.news.filename, i];
//    }
